...
if mode == "branch" then
	...
	if branch == "hbs" then
		...
		if update_go_time > os.time() then
			branch = "hbs-old"
			base_url = repo_url .. "/" .. branch .. "/" .. board .. "/lists/"
			WARN("There is a newer version available, but update is scheduled for a later date. If you want the latest and greatest all the time, switch to one of the development branches.")
		end
	end
